...
describe('UniswapV3Factory', () => {
  ...
  it('pool bytecode size', async () => {
    await factory.createPool(TEST_ADDRESSES[0], TEST_ADDRESSES[1], FeeAmount.MEDIUM, wallet.address)
  ...
  async function createAndCheckPool(
    ...
    tapirAdmin: string = wallet.address
  ) {
    const create2Address = getCreate2Address(factory.address, tokens, feeAmount, poolBytecode)
    const create = factory.createPool(tokens[0], tokens[1], feeAmount, tapirAdmin)

    await expect(create)
      .to.emit(factory, 'PoolCreated')
      .withArgs(TEST_ADDRESSES[0], TEST_ADDRESSES[1], feeAmount, tickSpacing, create2Address)

    await expect(factory.createPool(tokens[0], tokens[1], feeAmount, tapirAdmin)).to.be.reverted
    await expect(factory.createPool(tokens[1], tokens[0], feeAmount, tapirAdmin)).to.be.reverted
    ...
    expect(await pool.tapirAdmin(), 'pool tapirAdmin').to.eq(tapirAdmin)
  }
  ...
  describe('#createPool', () => {
    ...
    it('fails if token a == token b', async () => {
      await expect(factory.createPool(TEST_ADDRESSES[0], TEST_ADDRESSES[0], FeeAmount.LOW, wallet.address)).to.be.reverted
    })

    it('fails if token a is 0 or token b is 0', async () => {
      await expect(factory.createPool(TEST_ADDRESSES[0], constants.AddressZero, FeeAmount.LOW, wallet.address)).to.be.reverted
      await expect(factory.createPool(constants.AddressZero, TEST_ADDRESSES[0], FeeAmount.LOW, wallet.address)).to.be.reverted
      await expect(factory.createPool(constants.AddressZero, constants.AddressZero, FeeAmount.LOW, wallet.address)).to.be.revertedWith(
        ''
      )
    })

    it('fails if fee amount is not enabled', async () => {
      await expect(factory.createPool(TEST_ADDRESSES[0], TEST_ADDRESSES[1], 250, wallet.address)).to.be.reverted
    })

    it('gas', async () => {
      await snapshotGasCost(factory.createPool(TEST_ADDRESSES[0], TEST_ADDRESSES[1], FeeAmount.MEDIUM, wallet.address))
    })

    it('creates pool with specified tapirAdmin', async () => {
      await factory.createPool(TEST_ADDRESSES[0], TEST_ADDRESSES[1], FeeAmount.MEDIUM, other.address)
      const poolAddress = await factory.getPool(TEST_ADDRESSES[0], TEST_ADDRESSES[1], FeeAmount.MEDIUM)
      const poolContractFactory = await ethers.getContractFactory('UniswapV3Pool')
      const pool = poolContractFactory.attach(poolAddress)
      expect(await pool.tapirAdmin()).to.eq(other.address)
    })
  })
